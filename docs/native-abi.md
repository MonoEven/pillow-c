...
`pillow_c_image_open_png` and `pillow_c_image_save_png` keep PNG decode/encode inside the DLL through Windows Imaging Component. The current PNG path supports `L`, `RGB`, and `RGBA` image handles. Native open converts supported source PNG pixel formats into the DLL's row-major public modes, and native save writes valid PNG files from those modes after any required RGB/BGR channel packing inside C++.
