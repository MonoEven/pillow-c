...
`pillow_c_image_open_ppm` and `pillow_c_image_save_ppm` keep binary Netpbm PBM/PGM/PPM files in the DLL. The current PPM path supports `P4` bitmap as mode `1`, `P5` grayscale as mode `L`, and `P6` truecolor as mode `RGB`, requires `maxval=255` for `P5`/`P6`, accepts standard Netpbm whitespace and comments in headers, and writes Pillow-style `P4`/`P5`/`P6` headers for `1`, `L`, and `RGB` handles. `P4` bits are inverted relative to Pillow's external mode `1` raw bytes: Netpbm bit `1` is black and Pillow raw bit `1` is white. Plain-text `P1`/`P2`/`P3` and high-bit-depth Netpbm variants remain future surfaces.
