...
`ImageDraw.Draw(image).Rectangle(...)` mutates the image handle in place through a native rectangle path. Fill, outline, width, inclusive coordinates, clipping, and reversed-coordinate rejection are modeled from Pillow 11.3.0's `ImageDraw.rectangle` wrapper and `ImagingDrawRectangle` C implementation. The AHK facade only packs colors and dispatches one DLL call.
