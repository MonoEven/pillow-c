...
`ImageDraw.Draw(image).Rectangle(...)` mutates the image handle in place through a native rectangle path. Fill, outline, width, inclusive coordinates, clipping, and reversed-coordinate rejection are modeled from Pillow 11.3.0's `ImageDraw.rectangle` wrapper and `ImagingDrawRectangle` C implementation. `ImageDraw.Draw(image).Ellipse(...)` uses Pillow 11.3.0's integer span generator for fill and outline widths while keeping clipping in the native hot path. `ImageDraw.Draw(image).Arc(...)` and `ImageDraw.Draw(image).Chord(...)` reuse that span foundation plus Pillow's clip-ellipse half-plane tree, including angle normalization, full-circle delegation to ellipse paths, width handling, chord-line outlines, and clipping. `ImageDraw.Draw(image).Line(...)` covers Pillow's ordinary multi-segment line path, including `width <= 1` final endpoint draw, `width > 1` quadrilateral segment filling, and clipping behavior from `draw_lines`/`ImagingDrawLine`/`ImagingDrawWideLine`. `ImageDraw.Draw(image).Point(...)` batches Pillow's individual point writes, allowing empty and single-point coordinate lists while clipping out-of-bounds points. `ImageDraw.Draw(image).Polygon(...)` currently covers fill and `width <= 1` outline paths, including Pillow's scanline edge rules and closed outline behavior. Mask-assisted wide polygon outlines and `joint="curve"` are separate native surfaces. The AHK facade only packs colors and dispatches one DLL call for covered draw operations.
