...
`Image.open` and `image.save` expose native file-format paths for BMP, PNG, and JPEG. The BMP layer parses and writes uncompressed Windows BMP files, including Pillow-compatible 24-bit RGB save bytes, 8-bit grayscale BMP, and Pillow-style 32-bit RGBA BMP saves that reopen as RGB. The PNG layer keeps `L`, `LA`, `P`, `RGB`, and `RGBA` decode/encode work inside the DLL; WIC handles the common codec path while native chunk writing preserves Pillow-style `LA` and palette-mode PNG semantics. The JPEG layer keeps lossy `L` and `RGB` decode/encode in the DLL through WIC, with component-count probing so grayscale JPEGs reopen as `L` instead of being promoted by the wrapper.
